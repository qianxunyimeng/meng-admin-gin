...
type SysUser struct {
	...
	Status   string `json:"status" gorm:"default:1;comment:用户是否被冻结 1正常 0冻结"`
	...
	Editable string `json:"editable" gorm:"size:1;comment:是否可编辑，为了保证系统安全，一些内置数据禁止编辑和删除"`
	//DeptIds  []int  `json:"deptIds" gorm:"-"`
	//PostIds  []int  `json:"postIds" gorm:"-"`
	//RoleIds  []int  `json:"roleIds" gorm:"-"`
	Dept *SysDept `json:"dept"`
	Role *SysRole `json:"role" gorm:"-"`
	common.ControlBy
	common.ModelTime
}
...
func (e *SysUser) Generate() common.ActiveRecord {
	o := *e
	return &o
}
